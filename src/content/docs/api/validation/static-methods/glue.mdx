...
description: Used for validations which validity state depend on each other.
sidebar:
    label: .glue()
---
...
:::tip 
Since every "glued" `Validation` depends on the result of its own execution of a "glued" predicate it makes possible to "glue" [optional](/api/validation/constructor#parameters) validaitons with the non-optional (required).
:::
