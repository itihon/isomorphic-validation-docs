...
The effect control functions `cancelEffect()` and `setEffect()` returned by the `applyEffect()` function are intended to be used as [state callbacks](/concept/state-callbacks/) and accept a [`ValidationResult`](/api/validation-result/) as an argument. The `cancelEffect()` function cancels deferred execution of `effectFunction` and `setEffect()` does the same but also enqueues a new execution of `effectFunction`. The separate function for cancellation were introduced to provide more fine grained control for cases where you need to only cancel the previously deferred execution without settting a new one.
...
When pairs of effect control functions are created by different calls of the `applyEffect()` function with the same `effectID` passed in, functions returned from one call cancel `effectFunction` enqueued by `setEffect()` from another call, if the same `htmlElement` was passed as the `target` property in `ValidationResult`. In other words, separate calls of the `applyEffect()` function with the same `effectID` constitute a compound effect applied to the `htmlElement`.

```js
// the same htmlElement and effectID
// cancelEffect1, cancelEffect2 and setEffect2 
// will be cancelling the effect set by setEffect1
// cancelEffect2, cancelEffect1 and setEffect1 
// will be cancelling the effect set by setEffect2
...
When the function `createApplyEffect()` is being invoked on the server side, gives a warning and returns `applyEffect()` function which in turn will return an `Array` of two empty functions.
...
For examples, please refer to their source codes.